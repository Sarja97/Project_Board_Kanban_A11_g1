...
#PUNTO 1:  Realice la consulta correspondiente para crear la tabla Perro, teniendo en cuenta sus claves foraneas y primarias.
...
#PUNTO 2 Inserte en la tabla correspondiente un nuevo animal (perro) como paciente y el duenio asociado a ese animal.
...
#PUNTO 3 Borre un animal que ya no va a ser atendido. Para ello consulte antes en el historial, algun animal que ya no sea atendido desde hace mucho tiempo.

#SELECCION DEL ID_PERRO QUE FUE ATENTIDO HACE MAS TIEMPO
SELECT H.ID_PERRO, MAX(H.FECHA)
FROM HISTORIAL H
GROUP BY ID_PERRO
...
#PUNTO 4 Actualice la fecha de nacimiento de algun animal (perro) que usted considere.

UPDATE PERRO
SET FECHA_NAC = "2012-10-08"
WHERE ID_PERRO = 1;
